...
let InteractiveMap = function (selector, config = {}) {
        ...
        _this.widgets.resetBounds = new (L.Control.extend({
          ...
          onAdd: function (/* map */) {
            ...
            button.addEventListener("click", () => {
              ...
              _this.dispatchEvent(
                new CustomEvent(InteractiveMap.Event.ResetBounds.postReset, {
                  detail: null,
                })
              );
            });
  ...
  this.generateLayerTree = function (tree) {
      console.log("GENERATING LAYER TREE");
      console.log(tree);

      function recurseTree(branch, parentKey) {
          const {id, name, children, url, colour, style} = branch;
          const key = (parentKey ? `${parentKey}_${id}` : `${id}`).toLowerCase();
          console.log(id, name, children, url, colour, style);

          if (children) {
              return {
                  label: name,
                  children: recurseTree(children, key),
                  collapsed: true,
                  selectAllCheckbox: true,
              }
          } else if (url) {
              // const {rgba, colorBox} = generateColourValues(colour, style);

              console.log(url);
          }
      }

      // Generate layer tree control
      tree.forEach((branch) => {
          let result = recurseTree(branch, undefined);
   ...
   */
  this.load = function (layers, settings, layerTree) {
    settings = { ...InteractiveMap.defaultLayerSettings, ...settings };
    _this.generateLayerTree(layers);
    ...
    **/
    function recurseLayerTree(branch, parentKey) {
    ...
    layerTree.forEach((branch) => {
      let result = recurseLayerTree(branch, undefined);
      if (_this.config.widgets.includes("legend")) {
        layerTree.push(result);
      }
    });
...
InteractiveMap.prototype = {
  ...
  findFeature: function (
    ...
    // Turn the searchString into a list if it's a string
        ...
        layer.eachLayer(function (featureLayer) {
          ...
          for (const property of properties) {
            if (
              ...
              for (const searchStr of searchString) {
                // Store matched result feature
                if (searchProperty.includes(searchStr.toLowerCase())) {
...
InteractiveMap.Event = {
  ...
  /**
     * Reset Bounds Widget related events
     */
  ResetBounds: {
    /**
     * When the reset bounds has been executed.
     */
    postReset: "InteractiveMap.ResetBounds.postReset",
  }
};
